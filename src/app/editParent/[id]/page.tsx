...
export default function EditParent({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch single parent data and populate form
  useEffect(() => {
    if (!id) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `https://summer-bootcamp-apis.onrender.com/api/parents/${id}`,
        {
          method: "PATCH",
          body: formData,
        }
      ...
      >
        ...
        <section>
          ...
          <div className="space-y-3">
            ...
            <input
              ...
              required
            />
            <label>Image of Father</label>
            <input
              type="file"
              className="w-full p-2 border rounded bg-white"
              onChange={(e) =>
                setParentImage(e.target.files?.[0] || null)
              }
              accept="image/*"
            />
            <label>Image of Mother</label>
            <input
              type="file"
              className="w-full p-2 border rounded bg-white"
              onChange={(e) =>
        ...
        <section>
          ...
          <div className="space-y-3">
            ...
            />
            <label>Image of Guardian</label>
            <input
              type="file"
              className="w-full p-2 border rounded bg-white"
              onChange={(e) =>
                setThirdPartyImage(e.target.files?.[0] || null)
              }
              accept="image/*"
            />
          </div>
